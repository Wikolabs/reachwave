...
export default function DemoPage() {
  ...
  const t = lang === "fr" ? {
    ...
    poweredBy: "Modèle :",
    note: "DEMO POC, aucune connexion réelle Gmail, LinkedIn, Twilio. L'IA invente la sequence pour la démonstration.",
  } : {
  ...
  async function generate() {
    ...
    setLoading(true);
    try {
      const r = await fetch("/offers/reachwave/demo/api/demo", {
        method: "POST",
    ...
    <div style={{ minHeight: "100vh", background: PAL.bg, color: PAL.txt1, display: "flex", flexDirection: "column" }}>
      <style>{`
        *, *::before, *::after { box-sizing: border-box; }
        ...
        .wk-textarea { width: 100%; padding: 12px 14px; border-radius: 6px; background: ${PAL.surface}; border: 1px solid ${PAL.border}; color: ${PAL.txt1}; font-family: inherit; font-size: 14px; line-height: 1.55; resize: vertical; min-height: 110px; transition: border-color .2s, background .2s; }
      ...
      <main style={{ flex: 1, padding: "32px", maxWidth: 1200, margin: "0 auto", width: "100%" }}>
        ...
        <div className="demo-grid" style={{ display: "grid", gridTemplateColumns: "380px 1fr", gap: 24 }}>
          <section style={{ background: PAL.surface, border: `1px solid ${PAL.border}`, borderRadius: 6, padding: 22 }}>
            <h2 style={{ fontSize: "0.92rem", color: PAL.txt2, fontWeight: 700, margin: "0 0 10px" }}>{t.leadLabel}</h2>
            <textarea className="wk-textarea" value={lead} onChange={(e) => setLead(e.target.value)} placeholder={t.leadPlaceholder} />
            <h2 style={{ fontSize: "0.92rem", color: PAL.txt2, fontWeight: 700, margin: "16px 0 10px" }}>{t.offerLabel}</h2>
